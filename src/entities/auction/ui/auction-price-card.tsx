...
export function AuctionPriceCard({ trading }: { trading: AuctionDetailData['trading'] }) {
  ...
  const { current, available, min, max, step, price_per_km } = trading.price
  ...
  return (
    <Card>
      <CardContent>
        ...
        <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-slate-700 sm:grid-cols-5">
          <Field label="Доступная">{formatCurrency(available)}</Field>
